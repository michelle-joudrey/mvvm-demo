...
- (id)init {
  if (self = [super init]) {
    _viewModel = [MainViewModel newDemoInstance];
  }
  return self;
}
...
- (BOOL)application:(UIApplication *)application
  ...
  MainNavigationController *vc =
      (MainNavigationController *)self.window.rootViewController;
...
- (NSURL *)applicationDocumentsDirectory {
  ...
  // code uses a directory named "ricky.mvvm" in the application's documents
  // directory.
...
- (NSManagedObjectModel *)managedObjectModel {
  ...
  NSURL *modelURL =
      [[NSBundle mainBundle] URLForResource:@"mvvm" withExtension:@"momd"];
  _managedObjectModel =
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
  ...
  NSURL *storeURL = [[self applicationDocumentsDirectory]
      URLByAppendingPathComponent:@"mvvm.sqlite"];
  NSError *error = nil;
